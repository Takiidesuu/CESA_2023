Shader "Slime/Step7"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // 色の算出
            ocolor getColor(const float3 pos)
            {
                ocolor Ocolor;
            ...
            output frag(const v2f i)
            {
                ...
                for (int i = 0; i < 40; i++)
                {
                    ...
                    if (dist < 0.01)
                    {
                        ...
                        ocolor Ocolor = getColor(pos);
                        fixed3 baseColor = Ocolor.color;

                        const float rimPower = 2;
                        const float rimRate = pow(1 - abs(dot(norm, rayDir)), rimPower);
                        //発光度合い（元の値1.5)
                        const fixed3 rimColor = fixed3(0.2, -0.2, 0.8);
                        //const fixed3 rimColor = fixed3(2 - (Hardness / 5), -1.5 + (Hardness / 10), -1.5 + (Hardness / 10));
                        ...
                        //fixed3 color = clamp(lerp(baseColor, rimColor, rimRate) + (highlight/ 3), 0, 1); // 色

                        float alpha = clamp(lerp(0.4, 4, rimRate), 0, 1); // 不透明度
                        //float alpha = 0.4 + ((Hardness + 1) / 3);
                        if (alpha >= 1.0) {
                            alpha = 1.0f;
                        }
                        if (alpha <= 0.0) {
